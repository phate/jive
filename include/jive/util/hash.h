...
{ \
	size_t new_nbuckets = self->nbuckets * 2 + 1; \
	...
	for(n=0; n<self->nbuckets; n++) { \
		while(self->buckets[n].first) { \
			...
			size_t hashval = ((size_t) entry->key_member) % new_nbuckets; \
...
{ \
	...
	size_t hash = ((size_t) entry->key_member) % self->nbuckets; \
...
{ \
	if (!self->nbuckets) return 0; \
	 \
	size_t hash = ((size_t) keyval) % self->nbuckets; \
