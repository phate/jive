...
const jive_type_class JIVE_STACKSLOT_TYPE = {
	...
	.get_label = _jive_stackslot_type_get_label, /* override */
	.create_input = _jive_stackslot_type_create_input, /* override */
	.create_output = _jive_stackslot_type_create_output, /* override */
	.create_resource = _jive_stackslot_type_create_resource, /* override */
	.create_gate = _jive_stackslot_type_create_gate, /* override */
	.equals = _jive_stackslot_type_equals, /* override */
	.accepts = _jive_stackslot_type_accepts /* override */
};

const jive_input_class JIVE_STACKSLOT_INPUT = {
	.parent = &JIVE_INPUT,
	.fini = _jive_input_fini, /* inherit */
	.get_label = _jive_stackslot_input_get_label, /* override */
	.get_type = _jive_stackslot_input_get_type, /* override */
...
const jive_output_class JIVE_STACKSLOT_OUTPUT = {
	...
	.get_label = _jive_stackslot_output_get_label, /* override */
	.get_type = _jive_stackslot_output_get_type, /* override */
...
const jive_gate_class JIVE_STACKSLOT_GATE = {
	...
	.get_label = _jive_stackslot_gate_get_label, /* override */
	.get_type = _jive_stackslot_gate_get_type, /* override */
...
const jive_resource_class JIVE_STACKSLOT_RESOURCE = {
	...
	.fini = _jive_stackslot_resource_fini, /* override */
	.get_label = _jive_stackslot_resource_get_label, /* override */
	.get_type = _jive_stackslot_resource_get_type, /* override */
...
char *
_jive_stackslot_type_get_label(const jive_type * self_)
{
	const jive_stackslot_type * self = (const jive_stackslot_type *) self_;
	char tmp[80];
...
jive_input *
_jive_stackslot_type_create_input(const jive_type * self_, struct jive_node * node, size_t index, jive_output * initial_operand)
{
	const jive_stackslot_type * self = (const jive_stackslot_type *) self_;
	jive_stackslot_input * input = jive_context_malloc(node->graph->context, sizeof(*input));
	input->base.base.class_ = &JIVE_STACKSLOT_INPUT;
	_jive_stackslot_input_init(input, self->regcls, node, index, initial_operand);
	return &input->base.base;
}

jive_output *
_jive_stackslot_type_create_output(const jive_type * self_, struct jive_node * node, size_t index)
{
	const jive_stackslot_type * self = (const jive_stackslot_type *) self_;
	jive_stackslot_output * output = jive_context_malloc(node->graph->context, sizeof(*output));
	output->base.base.class_ = &JIVE_STACKSLOT_OUTPUT;
	_jive_stackslot_output_init(output, self->regcls, node, index);
	return &output->base.base;
}

jive_resource *
_jive_stackslot_type_create_resource(const jive_type * self_, struct jive_graph * graph)
{
	const jive_stackslot_type * self = (const jive_stackslot_type *) self_;
	jive_stackslot_resource * resource = jive_context_malloc(graph->context, sizeof(*resource));
	resource->base.base.class_ = &JIVE_STACKSLOT_RESOURCE;
	_jive_stackslot_resource_init(resource, self->regcls, graph);
	return &resource->base.base;
}

jive_gate *
_jive_stackslot_type_create_gate(const jive_type * self_, struct jive_graph * graph, const char * name)
{
	const jive_stackslot_type * self = (const jive_stackslot_type *) self_;
	jive_stackslot_gate * gate = jive_context_malloc(graph->context, sizeof(*gate));
	gate->base.base.class_ = &JIVE_STACKSLOT_GATE;
	_jive_stackslot_gate_init(gate, self->regcls, graph, name);
	return &gate->base.base;
}

bool
_jive_stackslot_type_equals(const jive_type * self_, const jive_type * other_)
{
	const jive_stackslot_type * self = (const jive_stackslot_type *) self_;
	const jive_stackslot_type * other = (const jive_stackslot_type *) other_;
	
	return _jive_type_equals(self_, other_) && (self->regcls == other->regcls);
}

bool
_jive_stackslot_type_accepts(const jive_type * self_, const jive_type * other_)
{
	const jive_stackslot_type * self = (const jive_stackslot_type *) self_;
	const jive_stackslot_type * other = (const jive_stackslot_type *) other_;
...
_jive_stackslot_input_init(jive_stackslot_input * self, const struct jive_regcls * regcls, struct jive_node * node, size_t index, jive_output * origin)
{
	...
	self->type = jive_stackslot_type_create(regcls);
}

char *
_jive_stackslot_input_get_label(const jive_input * self_)
{
	const jive_stackslot_input * self = (const jive_stackslot_input *) self_;
	char tmp[80];
...
const jive_type *
_jive_stackslot_input_get_type(const jive_input * self_)
{
	const jive_stackslot_input * self = (const jive_stackslot_input *) self_;
...
char *
_jive_stackslot_output_get_label(const jive_output * self_)
{
	const jive_stackslot_output * self = (const jive_stackslot_output *) self_;
	char tmp[80];
	snprintf(tmp, sizeof(tmp), "stack:%s", self->type.regcls->name);
	return strdup(tmp);
}
...
const jive_type *
_jive_stackslot_output_get_type(const jive_output * self_)
{
	const jive_stackslot_output * self = (const jive_stackslot_output *) self_;
	return &self->type.base.base;
}
...
void
_jive_stackslot_resource_fini(jive_resource * self_)
{
	jive_stackslot_resource * self = (jive_stackslot_resource *) self_;
	if (self->stackframe) JIVE_LIST_REMOVE(self->stackframe->slots, self, stackframe_slots_list);
...
char *
_jive_stackslot_resource_get_label(const jive_resource * self_)
{
	const jive_stackslot_resource * self = (const jive_stackslot_resource *) self_;
	char tmp[80];
	snprintf(tmp, sizeof(tmp), "stack:%s", self->type.regcls->name);
	return strdup(tmp);
}
...
const jive_type *
_jive_stackslot_resource_get_type(const jive_resource * self_)
{
	const jive_stackslot_resource * self = (const jive_stackslot_resource *) self_;
	return &self->type.base.base;
}
...
char *
_jive_stackslot_gate_get_label(const jive_gate * self_)
{
	const jive_stackslot_gate * self = (const jive_stackslot_gate *) self_;
	char tmp[80];
	snprintf(tmp, sizeof(tmp), "stack:%s", self->type.regcls->name);
	return strdup(tmp);
}
...
const jive_type *
_jive_stackslot_gate_get_type(const jive_gate * self_)
{
	const jive_stackslot_gate * self = (const jive_stackslot_gate *) self_;
	return &self->type.base.base;
}
