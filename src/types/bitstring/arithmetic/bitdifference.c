	...
	size_t noperands, jive_output * const operands[])
{
	if (!jive_output_isinstance(operands[0], &JIVE_BITSTRING_OUTPUT)){
		jive_context_fatal_error(region->graph->context, "Type mismatch: bitdifference node requires bitstring operands");
	}
...
{
	return jive_binary_operation_normalized_create(&JIVE_BITDIFFERENCE_NODE, region, NULL,
		2, (jive_output * []){op1, op2})->node;
}
