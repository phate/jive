	...
	size_t noperands, jive_output * const operands[])
{
	if (!jive_output_isinstance(operands[0], &JIVE_BITSTRING_OUTPUT)){
		jive_context_fatal_error(region->graph->context, "Type mismatch: bitshl node requires bitstring operands");
	}
...
jive_node *
jive_bitshl_create(jive_region * region, jive_output * operand, jive_output * shift)
{
	return jive_binary_operation_normalized_create(&JIVE_BITSHL_NODE, region, NULL,
