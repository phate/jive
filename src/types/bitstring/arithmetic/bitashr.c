	...
	size_t noperands, jive_output * const operands[])
{
	if (!jive_output_isinstance(operands[0], &JIVE_BITSTRING_OUTPUT)){
		jive_context_fatal_error(region->graph->context, "Type mismatch: bitashr node requires bitstring operands");
	}
	...
	jive_output * op1, jive_output * op2)
{
	...
	if(path == jive_binop_reduction_constants){
		...
		jive_bitstring_arithmetic_shiftright(bits, n1->attrs.bits, nbits, shift);
