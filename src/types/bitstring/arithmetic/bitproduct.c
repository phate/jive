	...
	size_t noperands, jive_output * const operands[])
{
	...
	jive_node * node = jive::create_operation_node(jive::bitstring::product_operation());
	...
	const jive_node_class * cls, const jive_node_attrs * attrs, jive_output * op1, jive_output * op2)
{
	if (path == jive_binop_reduction_constants) {
		...
		jive_bitstring_product(
			bits, nbits,
			&n1->operation().bits[0], nbits,
			&n2->operation().bits[0], nbits);
...
jive_bitmultiply(size_t noperands, jive_output * const * operands)
{
	...
	jive::bitstring::product_operation op;
	return jive_binary_operation_create_normalized(&JIVE_BITPRODUCT_NODE_.base, graph, &op,
		noperands, operands);
}
