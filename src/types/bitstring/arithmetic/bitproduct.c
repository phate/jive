	...
	size_t noperands, jive_output * const operands[])
{
	if (!jive_output_isinstance(operands[0], &JIVE_BITSTRING_OUTPUT)){
		jive_context_fatal_error(region->graph->context,
			"Type mismatch: bitproduct node requires bitstring operands");
	}
...
{
	return jive_binary_operation_create_normalized(&JIVE_BITPRODUCT_NODE_.base, region->graph,
		NULL, noperands, operands)->node;
}
