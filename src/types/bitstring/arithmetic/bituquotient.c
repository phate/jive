	...
	size_t noperands, jive_output * const operands[])
{
	if (!jive_output_isinstance(operands[0], &JIVE_BITSTRING_OUTPUT)){
		jive_context_fatal_error(region->graph->context, "Type mismatch: bituquotient node requires bitstring operands");
	}
...
jive_node *
jive_bituquotient_create(jive_region * region, jive_output * dividend, jive_output * divisor)
{
	return jive_binary_operation_normalized_create(&JIVE_BITUQUOTIENT_NODE, region, NULL,
