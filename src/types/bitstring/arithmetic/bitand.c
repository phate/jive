	...
	size_t noperands, jive_output * const operands[])
{
	if (!jive_output_isinstance(operands[0], &JIVE_BITSTRING_OUTPUT)){
		jive_context_fatal_error(region->graph->context, "Type mismatch: bitand node requires bitstring operands");
	}
...
jive_node *
jive_bitand_create(jive_region * region, size_t noperands, jive_output * operands[const])
{
	return jive_binary_operation_normalized_create(&JIVE_BITAND_NODE, region, NULL,
