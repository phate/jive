	...
	size_t noperands, jive_output * const operands[])
{
	...
	jive_node * node = jive::create_operation_node(jive::bitstring::xor_operation());
	...
	const jive_node_class * cls, const jive_node_attrs * attrs, jive_output * op1, jive_output * op2)
{
	...
	if (path == jive_binop_reduction_constants) {
		...
		char bits[nbits];
		jive_bitstring_xor(bits, &n1->operation().bits[0], &n2->operation().bits[0], nbits);
...
jive_bitxor(size_t noperands, jive_output * const * operands)
{
	...
	jive::bitstring::xor_operation op;
	return jive_binary_operation_create_normalized(&JIVE_BITXOR_NODE_.base, graph,
