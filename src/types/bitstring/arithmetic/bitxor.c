...
static jive_node *
jive_bitxor_create_(struct jive_region * region, const jive_node_attrs * attrs,
	size_t noperands, struct jive_output * const operands[]);

static jive_binop_reduction_path_t
jive_bitxor_node_can_reduce_operand_pair_(const jive_node_class * cls,
	const jive_node_attrs * attrs, const jive_output * op1, const jive_output * op2);

static jive_output *
jive_bitxor_node_reduce_operand_pair_(jive_binop_reduction_path_t path, const jive_node_class * cls,
...
const jive_bitbinary_operation_class JIVE_BITXOR_NODE_ = {
	.base = { /* jive_bitbinary_operation_class */
		.base = { /* jive_node_class */
			...
			.create = jive_bitxor_create_, /* override */
		...
		.can_reduce_operand_pair = jive_bitxor_node_can_reduce_operand_pair_, /* override */
		.reduce_operand_pair = jive_bitxor_node_reduce_operand_pair_ /* override */
	},
	.type = jive_bitop_code_xor
};

static void
jive_bitxor_node_init_(jive_node * self, jive_region * region,
	size_t noperands, jive_output * const operands[])
{
	if (!jive_output_isinstance(operands[0], &JIVE_BITSTRING_OUTPUT)){
		jive_context_fatal_error(region->graph->context,
			"Type mismatch: bitxor node requires bitstring operands");
	}
...
static jive_node *
jive_bitxor_create_(jive_region * region, const jive_node_attrs * attrs,
	size_t noperands, jive_output * const operands[])
{
	jive_node * node = jive_context_malloc(region->graph->context, sizeof(*node));
	node->class_ = &JIVE_BITXOR_NODE;
	jive_bitxor_node_init_(node, region, noperands, operands);
...
static jive_output *
jive_bitxor_node_reduce_operand_pair_(jive_binop_reduction_path_t path,
	const jive_node_class * cls, const jive_node_attrs * attrs, jive_output * op1, jive_output * op2)
{
	...
	if (path == jive_binop_reduction_constants) {
		...
		char bits[nbits];
		jive_bitstring_xor(bits, n1->attrs.bits, n2->attrs.bits, nbits);
...
{
	return jive_binary_operation_create_normalized(&JIVE_BITXOR_NODE_.base, region->graph, NULL,
...
jive_bitxor(size_t noperands, jive_output * const * operands)
{
	...
	return jive_binary_operation_create_normalized(&JIVE_BITXOR_NODE_.base, graph, NULL,
		noperands, operands);
}
