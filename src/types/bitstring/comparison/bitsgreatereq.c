	...
	jive_output * operand1, jive_output * operand2)
{
	if (!jive_output_isinstance(operand1, &JIVE_BITSTRING_OUTPUT)){
		jive_context_fatal_error(region->graph->context, "Type mismatch: bitsgreatereq node requires bitstring operands");
	}
...
jive_bitsgreatereq(jive_output * operand1, jive_output * operand2)
{
	...
	const jive_binary_operation_normal_form * nf = (const jive_binary_operation_normal_form *)
		jive_graph_get_nodeclass_form(region->graph, &JIVE_BITSGREATEREQ_NODE);
