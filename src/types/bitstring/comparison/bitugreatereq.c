	...
	jive_output * operand1, jive_output * operand2)
{
	if (!jive_output_isinstance(operand1, &JIVE_BITSTRING_OUTPUT)){
		jive_context_fatal_error(region->graph->context, "Type mismatch: bitugreatereq node requires bitstring operands");
	}
...
jive_node *
jive_bitugreatereq_create(jive_region * region,
	jive_output * operand1, jive_output * operand2)
{
	return jive_bitugreatereq_create_(region, NULL, 2, (jive_output *[]){operand1, operand2});
}
