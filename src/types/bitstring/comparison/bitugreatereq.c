	...
	size_t noperands, struct jive_output * const operands[])
{
	...
	jive_node * node = jive::create_operation_node(jive::bitstring::ugreatereq_operation());
...
jive_bitugreatereq(jive_output * operand1, jive_output * operand2)
{
	...
	jive::bitstring::ugreatereq_operation op;
	return jive_binary_operation_create_normalized(&JIVE_BITUGREATEREQ_NODE_.base, graph, &op, 2,
		tmparray2);
}
