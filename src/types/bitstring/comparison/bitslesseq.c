...
static jive_node *
jive_bitslesseq_create_(struct jive_region * region, const jive_node_attrs * attrs,
...
static jive_output *
jive_bitslesseq_node_reduce_operand_pair_(jive_binop_reduction_path_t path,
	const jive_node_class * cls, const jive_node_attrs * attrs, jive_output * op1,
	jive_output * op2);

const jive_bitcomparison_operation_class JIVE_BITSLESSEQ_NODE_ = {
	.base = { /* jive_binary_operation_class */
		.base = { /* jive_node_class */
			...
			.create = jive_bitslesseq_create_, /* override */
		...
		.can_reduce_operand_pair = jive_bitslesseq_node_can_reduce_operand_pair_, /* override */
		.reduce_operand_pair = jive_bitslesseq_node_reduce_operand_pair_ /* override */
	},
	.type = jive_bitcmp_code_slesseq,
	...
	jive_output * operand1, jive_output * operand2)
{
	if (!jive_output_isinstance(operand1, &JIVE_BITSTRING_OUTPUT)){
		jive_context_fatal_error(region->graph->context, "Type mismatch: bitslesseq node requires bitstring operands");
	}
	...
	size_t noperands, struct jive_output * const operands[])
{
	...
	node->class_ = &JIVE_BITSLESSEQ_NODE;
	jive_bitslesseq_node_init_(node, region, operands[0], operands[1]);
	...
	const jive_node_attrs * attrs, const jive_output * op1, const jive_output * op2)
{
	...
	if (n1 && n2){
		...
		char result = jive_bitstring_slesseq(n1->attrs.bits, n2->attrs.bits,
			n1->attrs.nbits);
...
jive_node *
jive_bitslesseq_create(jive_region * region,
	jive_output * operand1, jive_output * operand2)
{
	return jive_bitslesseq_create_(region, NULL, 2, (jive_output *[]){operand1, operand2});
}

jive_output *
jive_bitslesseq(jive_output * operand1, jive_output * operand2)
{
	jive_graph * graph = operand1->node->graph;
	return jive_binary_operation_create_normalized(&JIVE_BITSLESSEQ_NODE_.base, graph, NULL, 2,
