	...
	jive_output * operand1, jive_output * operand2)
{
	if (!jive_output_isinstance(operand1, &JIVE_BITSTRING_OUTPUT)){
		jive_context_fatal_error(region->graph->context, "Type mismatch: bituless node requires bitstring operands");
	}
	...
	const jive_node_attrs * attrs, const jive_output * op1, const jive_output * op2)
{
	...
	if (n1 && n2){
		...
		char result = jive_bitstring_uless(n1->attrs.bits, n2->attrs.bits,
			n1->attrs.nbits);
	...
	if (n1){
		...
		if (jive_bitstring_equal(n1->attrs.bits, uint_max, nbits) == '1')
			return 3;
	}
	...
	if (n2){
		...
		if (jive_bitstring_equal(n2->attrs.bits, uint_min, nbits) == '1')
			return 4;
	}
...
jive_bituless(jive_output * operand1, jive_output * operand2)
{
	...
	return jive_binary_operation_create_normalized(&JIVE_BITULESS_NODE_.base, graph, NULL, 2,
