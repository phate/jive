...
notequal_operation::~notequal_operation() noexcept {}

bool
notequal_operation::operator==(const operation & other) const noexcept
{
	const notequal_operation * o = dynamic_cast<const notequal_operation *>(&other);
	return o && o->type() == type();
}

jive_node *
notequal_operation::create_node(
	jive_region * region,
	size_t narguments,
	jive::output * const arguments[]) const
{
	return detail::binop_create<notequal_operation>(
		*this,
...
compare_result
notequal_operation::reduce_constants(
...
notequal_operation::flags() const noexcept
{
	return jive_binary_operation_commutative;
}

std::string
notequal_operation::debug_string() const
{
	return "BITNOTEQUAL";
}
...
jive_bitnotequal(jive::output * operand1, jive::output * operand2)
{
	...
	return jive::bitstring::detail::binop_normalized_create<
		jive::bitstring::notequal_operation>(
			&JIVE_BITNOTEQUAL_NODE_.base, operand1, operand2);
}
