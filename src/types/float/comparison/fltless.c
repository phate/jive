...
static jive_node *
jive_fltless_node_create_(struct jive_region * region, const jive_node_attrs * attrs,
...
const jive_fltcomparison_operation_class JIVE_FLTLESS_NODE_ = {
	base : { /* jive_binary_operation_class */
		base : { /* jive_node_class */
			...
			create : jive_fltless_node_create_, /* override */
		},
	...
	size_t noperands, struct jive_output * const operands[])
{
	...
	jive_node * node = jive::create_operation_node(jive::flt::less_operation());
	node->class_ = &JIVE_FLTLESS_NODE;
	jive_fltless_node_init_(node, region, operands[0], operands[1]);

	return node;
}

jive_output *
jive_fltless(jive_output * op1, jive_output * op2)
{
	jive_graph * graph = op1->node()->graph;
	jive_output * tmparray2[] = {op1, op2};
	jive::flt::less_operation op;
	return jive_binary_operation_create_normalized(&JIVE_FLTLESS_NODE_.base, graph, &op, 2,
		tmparray2);
}
