	...
	size_t noperands, struct jive_output * const operands[])
{
	...
	jive_rlgreater_node * node = new jive_rlgreater_node;
	node->class_ = &JIVE_RLGREATER_NODE;
...
jive_rlgreater(jive_output * op1, jive_output * op2)
{
	...
	return jive_binary_operation_create_normalized(&JIVE_RLGREATER_NODE_.base, graph, NULL,
		2, tmparray2);
}
