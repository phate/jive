...
node_selection_order::node_selection_order(master_selector_cost * master, jive_node * node)
	: state_(state_ahead)
...
/* region_selector_cost */

region_selector_cost::~region_selector_cost() noexcept {}

region_selector_cost::region_selector_cost(
	master_selector_cost * master,
...
void
region_selector_cost::push_node_stack_internal(node_selection_order * node_cost)
{
...
void
region_selector_cost::push_node_stack(
	jive_node * node)
{
...
jive_node *
region_selector_cost::select_node()
{
...
void
region_selector_cost::add_node_output_ssavars(
	jive_node * node,
...
node_selection_order *
region_selector_cost::map_node_internal(jive_node * node) const
{
	return master_->map_node_internal(node);
}

std::vector<jive_ssavar *>
region_selector_cost::prio_sorted_ssavars() const
{
...
jive_ssavar *
region_selector_cost::select_spill(
...
/* master_selector_cost */

master_selector_cost::~master_selector_cost()
{
}

master_selector_cost::master_selector_cost(jive_shaped_graph * shaped_graph)
	...
	, cost_computation_state_tracker_(&shaped_graph->graph())
{
	...
	callbacks_.push_back(shaped_graph->graph().on_node_create.connect(
		std::bind(&master_selector_cost::handle_node_create, this, _1)));
	callbacks_.push_back(shaped_graph->graph().on_input_change.connect(
		std::bind(&master_selector_cost::handle_input_change, this, _1, _2, _3)));
...
region_selector_cost *
master_selector_cost::map_region(const jive_region * region)
{
	region_selector_cost * region_selector_cost = nullptr;
	auto i = region_map_.find(region);
	if (i != region_map_.end())
		region_selector_cost = i.ptr();

	if (!region_selector_cost) {
		jive_shaped_region * shaped_region = shaped_graph_->map_region(region);
		region_selector_cost = region_shaper_selector_create(region, shaped_region);
	}
	return region_selector_cost;
}

node_selection_order *
master_selector_cost::map_node(jive_node * node)
{
	revalidate();
...
bool
master_selector_cost::check_node_selectable(const jive_node * node)
{
	for (size_t n = 0; n < node->noutputs; n++) {
...
node_selection_order *
master_selector_cost::map_node_internal(jive_node * node)
{
...
void
master_selector_cost::invalidate_node(jive_node * node)
{
	cost_computation_state_tracker_.invalidate(node);
}

void
master_selector_cost::revalidate_node(jive_node * node)
{
	...
	if (changes) {
		region_selector_cost * region_shaper;
...
void
master_selector_cost::revalidate()
{
	jive_node * node;
...
bool
master_selector_cost::assumed_active(
...
bool
master_selector_cost::maybe_inner_node(const jive_node * node) const noexcept
{
...
void
master_selector_cost::compute_node_cost(
	jive_resource_class_count * cost,
	jive_node * node)
{
	if (shaped_graph_->is_node_placed(node)) {
...
master_selector_cost::compute_blocked_rescls_priority(jive_node * node)
{
	if (shaped_graph_->is_node_placed(node)) {
...
void
master_selector_cost::try_add_frontier(jive_node * node)
{
...
bool
master_selector_cost::remove_frontier(jive_node * node)
{
	node_selection_order * node_cost = map_node_internal(node);
...
void
master_selector_cost::mark_shaped(jive_node * node)
{
	node_selection_order * node_cost = map_node_internal(node);
	
	region_selector_cost * region_shaper = map_region(node->region);
	switch (node_cost->state_) {
		case node_selection_order::state_queue: {
			region_shaper->node_queue_.erase(node_cost->queue_index_);
			break;
...
void
master_selector_cost::init_region_recursive(jive_region * region)
{
	jive_node * node;
...
node_selection_order *
master_selector_cost::node_cost_create(jive_node * node)
{
...
region_selector_cost *
master_selector_cost::region_shaper_selector_create(
	const jive_region * region,
	const jive_shaped_region * shaped_region)
{
	std::unique_ptr<region_selector_cost> self(
		new region_selector_cost(this, region, shaped_region));

	return region_map_.insert(std::move(self)).ptr();
}

void
master_selector_cost::handle_node_create(jive_node * node)
{
	bool stacked = false;
	for (size_t n = 0; n < node->ninputs; n++) {
		input * in = node->inputs[n];
...
void
master_selector_cost::handle_input_change(
