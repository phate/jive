	...
	size_t nreturns, const jive_argument_type returns[])
{
	...
	for (n = 0; n < nparameters; n++) {
		...
		cls = jive_fixed_stackslot_class_get(32, (n + 1) * 4);
	...
	self->stackptr = jive_subroutine_create_passthrough(&self->base, &jive_i386_regcls[jive_i386_gpr_esp].base, "stackptr");
	...
	const jive_resource_class * stackslot_cls = jive_fixed_stackslot_class_get(32, 0);
