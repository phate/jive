...
typedef struct jive_cone_traverser jive_cone_traverser;

struct jive_cone_traverser {
	jive_traverser base;
...
jive_unshaped_downward_cone_traverser_add_frontier(jive_cone_traverser * self, jive_node * node)
{
	...
	JIVE_LIST_ITERATE(self->base.frontier, insertion_point, traverser_node_list)
		if (insertion_point->node->depth_from_root < node->depth_from_root) break;
	
	JIVE_LIST_INSERT(self->base.frontier, insertion_point, nodestate, traverser_node_list);
}

static void
jive_unshaped_downward_cone_traverser_add_below(jive_cone_traverser * self, jive_node * node)
{
	size_t n;
	for(n=0; n<node->noutputs; n++) {
		jive_input * user;
		JIVE_LIST_ITERATE(node->outputs[n]->users, user, output_users_list) {
			if (!user->node->shape_location) jive_unshaped_downward_cone_traverser_add_frontier(self, user->node);
		}
	}
}

static jive_node *
_jive_unshaped_downward_cone_traverser_next(jive_traverser * self_)
{
	jive_cone_traverser * self = (jive_cone_traverser *) self_;
	if (!self->base.frontier.last) return 0;
	
	jive_traversal_nodestate * nodestate = self->base.frontier.last;
	jive_traversal_state_mark_behind(&self->state_tracker, nodestate);
	jive_node * node = nodestate->node;
	
	jive_unshaped_downward_cone_traverser_add_below(self, node);
	
	return node;
}

static void
_jive_unshaped_downward_cone_traverser_fini(jive_traverser * self_)
{
	jive_cone_traverser * self = (jive_cone_traverser *) self_;
	jive_traversal_state_fini(&self->state_tracker);
	_jive_traverser_fini(&self->base);
...
static const jive_traverser_class JIVE_UNSHAPED_DOWNWARD_CONE_TRAVERSER = {
	.parent = &JIVE_TRAVERSER,
	.fini = _jive_unshaped_downward_cone_traverser_fini,
	.next = _jive_unshaped_downward_cone_traverser_next
};

static void
_jive_unshaped_downward_cone_traverser_init(jive_cone_traverser * self, jive_node * node)
{
	_jive_traverser_init(&self->base, node->region->graph);
	jive_traversal_state_init(&self->state_tracker, node->graph);
	jive_unshaped_downward_cone_traverser_add_below(self, node);
}
...
{
	jive_cone_traverser * self = jive_context_malloc(node->region->graph->context, sizeof(*self));
	_jive_unshaped_downward_cone_traverser_init(self, node);
