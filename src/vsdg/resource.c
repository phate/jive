...
jive_resource_class_count_lookup_item(const jive_resource_class_count * self,
...
{
	jive_resource_class_count_item * item = jive_resource_class_count_lookup_item(self,
		resource_class);
	if (item != 0)
		return item->count;
	else
...
static size_t
jive_resource_class_count_add_single(jive_resource_class_count * self,
	const jive_resource_class * resource_class, size_t count)
{
	jive_resource_class_count_item * item = jive_resource_class_count_lookup_item(self,
		resource_class);
...
static void
jive_resource_class_count_sub_single(jive_resource_class_count * self,
	const jive_resource_class * resource_class)
{
	jive_resource_class_count_item * item = jive_resource_class_count_lookup_item(self,
		resource_class);
...
jive_resource_class_count_check_change(const jive_resource_class_count * self,
	const jive_resource_class * old_resource_class, const jive_resource_class * new_resource_class)
{
	if (!old_resource_class) return jive_resource_class_count_check_add(self, new_resource_class);
	if (!new_resource_class) return 0;
	
	const jive_resource_class * common_resource_class = jive_resource_class_union(old_resource_class,
		new_resource_class);
	...
	const jive_resource_class_count * other)
{
	...
	struct jive_resource_class_count_iterator i = jive_resource_class_count_begin(self);
	for (; i.entry; jive_resource_class_count_iterator_next(&i)) {
