...
jive_binary_operation_get_default_normal_form_(const jive_node_class * cls, jive_node_normal_form * parent_, struct jive_graph * graph)
{
...
jive_binary_operation_normalize_node_(const jive_node_normal_form * self_, jive_node * node)
{
	...
	const jive_binary_operation_class * cls = (const jive_binary_operation_class *) self->base.node_class;
...
bool
jive_binary_operation_operands_are_normalized_(const jive_node_normal_form * self_, size_t noperands, jive_output * const operands[], const jive_node_attrs * attrs)
{
	const jive_binary_operation_normal_form * self = (const jive_binary_operation_normal_form *) self_;
	if (!self->base.enable_mutable)
		return true;
	...
	size_t noperands_, jive_output * const operands_[])
{
	...
	/* possibly expand associative */
	if (self->base.enable_mutable && self->enable_flatten && (cls->flags & jive_binary_operation_associative)) {
		size_t count = 0, n;
