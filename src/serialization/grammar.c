	...
	jive_token_istream * is, jive_graph * graph, jive::gate ** gate)
{
	...
	*gate = graph->create_gate(*type, name);
	...
	jive::node * node, jive_token_ostream * os)
{
	...
	for (n = 0; n < node->ninputs(); ++n) {
		jive_portinfo port;
		port.origin = dynamic_cast<jive::output*>(node->input(n)->origin());
	...
	jive_token_istream * is, jive::region * region, jive::node ** node)
{
	...
	*node = jive_opnode_create(*op, region, origins);
		...
		for (k = 0; k < level->items.size(); ++k) {
			...
			for (j = 0; j < node->ninputs(); ++j) {
				...
				if (dynamic_cast<const jive::achr::type*>(&input->type()))
					jive_serialize_regiondef(self, namegen, input->origin()->region(), os);
			}
