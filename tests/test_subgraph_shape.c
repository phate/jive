...
int main()
{
	...
	jive_node * top = jive_node_create(region,
		0, NULL, NULL,
		3, (const jive_type *[]){&jive_type_singleton, &jive_type_singleton, &jive_type_singleton});
	
	jive_node * cmp = jive_node_create(region,
		2, (const jive_type *[]){&jive_type_singleton, &jive_type_singleton},(jive_output *[]){top->outputs[0], top->outputs[1]},
	...
	jive_node * alt1 = jive_node_create(sub1,
		1, (const jive_type *[]){&jive_type_singleton}, (jive_output *[]){top->outputs[0]},
		1, (const jive_type *[]){&jive_control_type_singleton});
	
	jive_region * sub2 = jive_region_create_subregion(region);
	jive_node * alt2 = jive_node_create(sub2,
		1, (const jive_type *[]){&jive_type_singleton}, (jive_output *[]){top->outputs[1]},
		1, (const jive_type *[]){&jive_control_type_singleton});
	
	jive_node * gamma = jive_node_create(region,
		3, (const jive_type *[]){&jive_type_singleton, &jive_control_type_singleton, &jive_control_type_singleton},(jive_output *[]){cmp->outputs[0], alt1->outputs[0], alt2->outputs[0]},
		1, (const jive_type *[]){&jive_type_singleton});
	
	jive_node * bottom = jive_node_create(region,
		2, (const jive_type *[]){&jive_type_singleton, &jive_type_singleton}, (jive_output *[]){gamma->outputs[0], top->outputs[2]},
		0, NULL);
	...
	jive_resource * r4 = jive_type_create_resource(&jive_type_singleton, graph);
