...
static int test_main(void)
{
	...
	jive_output * or0 = jive_bitor(2, (jive_output *[]){top->outputs[0], top->outputs[1]});
	jive_output * or1 = jive_bitor(2, (jive_output *[]){c0, c1});
	
	jive_node * bottom = jive_node_create(graph->root_region,
		2, (const jive_type *[]){bits32, bits32}, (jive_output *[]){or0, or1}, 0, NULL);
	jive_node_reserve(bottom);

	jive_graph_prune(graph);
	jive_view(graph, stdout);

	assert(jive_node_isinstance(bottom->inputs[0]->origin->node, &JIVE_BITOR_NODE));
