...
static int test_main(void)
{
	...
	jive_output * product0 = jive_bitmultiply(2, (jive_output *[]){top->outputs[0],
		top->outputs[1]});
	jive_output * product1 = jive_bitmultiply(2, (jive_output *[]){c0, c1});

	jive_node * bottom = jive_node_create(graph->root_region,
		2, (const jive_type *[]){bits32, bits32}, (jive_output *[]){product0, product1}, 0, NULL);
	...
	assert(jive_node_isinstance(bottom->inputs[0]->origin->node, &JIVE_BITPRODUCT_NODE));
	...
	assert(jive_bitconstant_equals_unsigned(bc1, 15));
