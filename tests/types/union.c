...
static int test_unnchoose(void)
{
	...
	auto top = jive::test::node_create(graph.root(), {}, {}, {&bits8, &unntype, &unntype, &addrtype});
...
static int test_unnunify(void)
{
	...
	auto top = jive::test::node_create(graph.root(), {}, {}, {&bits8});
	...
	auto bottom = jive::test::node_create(graph.root(), {&unntype, &unntype_empty}, {u0, u1},
		{&bits8});
	graph.export_port(bottom->output(0), "dummy");
