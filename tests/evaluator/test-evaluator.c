...
*/
	...
	jive::output * i = jive_bitconstant_unsigned(graph, 32, 1);
	jive::output * j = jive_bitconstant_unsigned(graph, 32, 0);
	jive::output * k = jive_bitconstant_unsigned(graph, 32, 1);
	...
	jive::output * one = jive_bitconstant_unsigned(graph, 32, 1);
...
*/
	...
	jive::output * n = lambda->arguments[0];
	jive::output * one = jive_bitconstant_unsigned(graph, 32, 1);
	jive::output * two = jive_bitconstant_unsigned(graph, 32, 2);
...
test_loadstore(struct jive_graph * graph)
{
	...
	jive_lambda * lambda = jive_lambda_begin(graph->root_region, 2, &types[0], &names[0]);
	...
	jive::output * three = jive_bitconstant_unsigned(graph, 4, 3);
